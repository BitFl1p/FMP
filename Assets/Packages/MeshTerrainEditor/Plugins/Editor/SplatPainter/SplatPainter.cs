...
namespace MTE
{
    ...
    {
        private enum SplatTexturePainterMode
        {
            FilteredGameObjects,
            SelectedGameObject
        }
        private static readonly GUIContent[] PainterModeContents =
        {
        ...
        #region Constant
        // default
        const SplatTexturePainterMode DefaultPainterMode
            = SplatTexturePainterMode.FilteredGameObjects;
        ...
        private SplatTexturePainterMode painterMode;
        ...
        public float BrushSize
        {
            ...
            set
            {
                ...
                if (!MathEx.AmostEqual(brushSize, value))
                {
                    ...
                    if (PainterMode == SplatTexturePainterMode.FilteredGameObjects)
                    {
        ...
        public float BrushAlpha
        {
            ...
            set
            {
                if (Mathf.Abs(brushAlpha - value) > 0.0001f)
                {
                    brushAlpha = value;
                    EditorPrefs.SetFloat("MTE_SplatPainter.brushAlpha", value);
                }
            }
        }
        ...
        public SplatPainter()
        {
            MTEContext.EnableEvent += (sender, args) =>
            {
                if (MTEContext.editor == this)
                {
                    LoadSavedParamter();
                    LoadTextureList();
                    if (PainterMode == SplatTexturePainterMode.SelectedGameObject)
                    {
        ...
        private void LoadPreview()
        {
            if (PainterMode == SplatTexturePainterMode.SelectedGameObject)
            {
                preview.LoadPreview(TextureList[SelectedTextureIndex],
                    BrushSizeInU3D, BrushIndex, targetGameObject);
            }
            else
            {
                preview.LoadPreview(TextureList[SelectedTextureIndex],
        ...
        private void LoadSavedParamter()
        {
            ...
            painterMode = (SplatTexturePainterMode)EditorPrefs.GetInt(
            ...
            brushAlpha = EditorPrefs.GetFloat("MTE_SplatPainter.brushAlpha", DefaultBrushAlpha);
        }
        ...
        public void DoArgsGUI()
        {
            ...
            this.PainterMode = (SplatTexturePainterMode)GUILayout.Toolbar(
                (int)this.PainterMode, PainterModeContents);
            if (EditorGUI.EndChangeCheck()
                && PainterMode == SplatTexturePainterMode.SelectedGameObject)
            {
                BuildEditingInfoForLegacyMode(Selection.activeGameObject);
            }
            if (PainterMode == SplatTexturePainterMode.SelectedGameObject
                        ...
                        EditorGUILayout.BeginHorizontal();
                        {
                            ...
                            for (int j = 0; j < 4; j++)
                            {
                                ...
                                if (new_toggleOn && !toggleOn)
                                {
                                    ...
                                    if (PainterMode == SplatTexturePainterMode.SelectedGameObject)
                                    {
                                        preview.LoadPreview(texture, BrushSizeInU3D, BrushIndex, targetGameObject);
        ...
        BuiltinPreview builtinPreview = new BuiltinPreview();
        SRPPreview srpPreview = new SRPPreview();
