...
namespace MTE
{
    internal class ObjectPainter : IEditor
    {
        ...
        {
            /*
            //Unity-issue: collider is not in world space as documented here:
            ...
            */
            ...
            return new Bounds(new Vector3(float.MaxValue, float.MaxValue, float.MaxValue),
                Vector3.zero);
        }
            ...
            else//loop through all object listed in detail
            {
                ...
                foreach (var o in objects)
                {
                    ...
                    foreach (var objectDetail in detailList)
                    {
                        ...
                        if (!Utility.IsInstanceOfPrefab(o, prefab))
                        {
                            continue;
                        }
        ...
        private void RemoveObjectInstances(List<GameObject> itemsRemoved)
        {
            ...
            {
                var prefab = Utility.GetPrefabRoot(instance);
                if (!prefab)
        ...
        private static void GetObjectItemsInCircle(GameObject prefab, Vector3 center, float radius, List<GameObject> result)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                if (Utility.IsPrefab(obj) && Utility.IsInstanceOfPrefab(obj, prefab))
                {
